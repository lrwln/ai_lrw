...
能自己点格子放草、树、房子等，搭一个属于自己的迷你村庄。

## Prompt 
```
我想做一个网页上的”3D小世界编辑器“，定位是”摆在桌子上的小模型“那种感觉，不是开放世界游戏。
## 我想要的体验
- 一开打网页能看到一个 8x8 左右的世界，立刻能完
...
- 一个”清空“按钮全变草地
- 首次打开有个简短操作提示（”拖拽旋转、滚轮缩放、点子放置“之类），几秒后自动淡出。
## 技术架构
...
豆包和gpt 模型 差异，复杂任务需要高级模型

## LLM 
Large Laguage Model 
大模型是依托海量数据训练（微软75亿美金收购github, meta 收购亚历山大王 148亿美金
 49%股份 Scale AI 数据标注/数据训练 清洗），参数量庞大的深度神经网络模型（DNN 
 Deep Neural Network ）, 逻辑推理与内容生成能力（生成代码），可适配多类复杂智能任务。

- deepseek 最新模型参数规模？  适配了华为芯片 
    deepseek-v4-pro(贵 ) 1.6万亿   1600b
    deepseek-v4-Flash(性价比) 2840亿  284b  

## ollama
本地大模型管理平台 ，安装一些开源大模型（芯片、显卡）。
具身智能（机器人） 工业 4.0 万兆宽带， 本地算力（黑盒子硬件，理想livus 汽车）


大模型就是由海量权重**参数**构建的概率映射**函数**，输入语义序列， 通过参数运算
拟合语言规律，生成连贯合理的输出结果。
LLM 数学公式
